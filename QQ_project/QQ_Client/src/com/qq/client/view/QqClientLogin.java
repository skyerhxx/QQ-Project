...
public class QqClientLogin extends JFrame implements ActionListener{
    ...
    public QqClientLogin(){
        ...
        //把控件按照顺序加入到jp2
        jp2.add(jp2_jbl1); //qq号
        jp2.add(jp2_jtf);  //qq号输入框
        jp2.add(jp2_jb1);  //清除号码
        jp2.add(jp2_jbl2); //qq密码
        jp2.add(jp2_jpf);  //qq密码输入框
        jp2.add(jp2_jbl3); //忘记密码
        jp2.add(jp2_jcb1); //隐身登录
        jp2.add(jp2_jcb2); //记住密码
        jp2.add(jp2_jbl4); //申请密码保护
        //创建选项卡窗口
        jtp = new JTabbedPane();
        jtp.add("QQ号码",jp2);
        jp3 = new JPanel();
    ...
    public void actionPerformed(ActionEvent arg0) {
        ...
        if(arg0.getSource()==jp1_jb1){
            ...
            if(qqClientUser.checkUser(u))  //这里判断用户是否合法，账号密码是否正确
            {
                //发送一个要求返回在线好友的请求包
                try {
                    //把创建好友列表的语句提前
